...
## focus on HIV negative ( 2 levels only)
...
kx1de[1:50,1:6] |>
  kbl(caption="KIR3DL2 associated genes in HIV- cells (low vs high)") |>
  kable_paper("hover", full_width = F)

write.table(x=kx1de,file="KIR3DL2_de_hivneg_2LB.tsv",sep="\t",quote=FALSE)

## focus on HIV negative ( 3 levels)
kdf1 <- subset(kdf,hiv_status==0)
kx1 <- kx[,which(colnames(kx) %in% rownames(kdf1))]
colSums(kx1)
plotMDS(kx1)
...
kx1de[1:50,1:6] |>
  kbl(caption="KIR3DL2 associated genes in HIV- cells (3 levels)") |>
  kable_paper("hover", full_width = F)

write.table(x=kx1de,file="KIR3DL2_de_hivneg_3LB.tsv",sep="\t",quote=FALSE)

## focus on HIV negative - 2 levels only - use sample pairing
kdf1 <- subset(kdf,hiv_status==0 & KIR3DL2!=2)
kx1 <- kx[,which(colnames(kx) %in% rownames(kdf1))]
colSums(kx1)
plotMDS(kx1)
...
dds <- DESeqDataSetFromMatrix(countData = kx1f , colData = kdf1, design = ~ patient_id + KIR3DL2)
res <- DESeq(dds)
z <- results(res)
vsd <- vst(dds, blind=FALSE)
zz <- cbind(as.data.frame(z),assay(vsd),kx1f)
...
kx1de[1:50,1:6] |>
  kbl(caption="KIR3DL2 associated genes in HIV- cells (low vs high) with patient level pairing") |>
  kable_paper("hover", full_width = F)

write.table(x=kx1de,file="KIR3DL2_de_hivneg_2LP.tsv",sep="\t",quote=FALSE)

## focus on HIV negative - 3 levels - use sample pairing
kdf1 <- subset(kdf,hiv_status==0 )
kx1 <- kx[,which(colnames(kx) %in% rownames(kdf1))]
colSums(kx1)
plotMDS(kx1)
...
kx1de[1:50,1:6] |>
  kbl(caption="KIR3DL2 associated genes in HIV- cells (3 levels) with patient level pairing") |>
  kable_paper("hover", full_width = F)

write.table(x=kx1de,file="KIR3DL2_de_hivneg_3LP.tsv",sep="\t",quote=FALSE)
...
kx2de[1:50,1:6] |> kbl(caption="KIR3DL2 associated genes in HIV+ cells - low vs high - batch corrected") |> kable_paper("hover", full_width = F)

write.table(x=kx2de,file="KIR3DL2_de_hivpos_2LB.tsv",sep="\t",quote=FALSE)

## 3 levels of KIR3DL2 expression
kdf2 <- subset(kdf,hiv_status==1 )
kx2 <- kx[,which(colnames(kx) %in% rownames(kdf2))]
colSums(kx2)
plotMDS(kx2)
...
kx2de[1:50,1:6] |> kbl(caption="KIR3DL2 associated genes in HIV+ cells - 3 levels") |> kable_paper("hover", full_width = F)

write.table(x=kx2de,file="KIR3DL2_de_hivpos_3LB.tsv",sep="\t",quote=FALSE)

## 2 levels - low vs high - using patient level pairing
kdf2 <- subset(kdf,hiv_status==1 & KIR3DL2!=2)
kx2 <- kx[,which(colnames(kx) %in% rownames(kdf2))]
colSums(kx2)
plotMDS(kx2)
...
kx2f <- kx2[which(rowMeans(kx2)>=5),]
kx2f <- kx2f+1
dds <- DESeqDataSetFromMatrix(countData = kx2f , colData = kdf2, design = ~ patient_id + KIR3DL2)
res <- DESeq(dds)
z <- results(res)
vsd <- vst(dds, blind=FALSE)
zz <- cbind(as.data.frame(z),assay(vsd),kx2f)
...
kx2de[1:50,1:6] |> kbl(caption="KIR3DL2 associated genes in HIV+ cells - low vs high with patient level pairing") |> kable_paper("hover", full_width = F)

write.table(x=kx2de,file="KIR3DL2_de_hivpos_2LP.tsv",sep="\t",quote=FALSE)

## 3 levels - low meium high - using patient level pairing
kdf2 <- subset(kdf,hiv_status==1 )
kx2 <- kx[,which(colnames(kx) %in% rownames(kdf2))]
colSums(kx2)
plotMDS(kx2)
...
kx2de[1:50,1:6] |> kbl(caption="KIR3DL2 associated genes in HIV+ cells - 3 level with patient level pairing") |> kable_paper("hover", full_width = F)

write.table(x=kx2de,file="KIR3DL2_de_hivpos_3LP.tsv",sep="\t",quote=FALSE)
...
kx3de[1:50,1:6] |> kbl(caption="KIR3DL2 associated genes irrespective of HIV status - 2 levels - batch correction") |> kable_paper("hover", full_width = F)

write.table(x=kx2de,file="KIR3DL2_de_hivall_2LB.tsv",sep="\t",quote=FALSE)
...
kx3de[1:50,1:6] |> kbl(caption="KIR3DL2 associated genes irrespective of HIV status - 3 levels - batch correction") |> kable_paper("hover", full_width = F)

write.table(x=kx2de,file="KIR3DL2_de_hivall_3LB.tsv",sep="\t",quote=FALSE)

# 2 levels with patient level pairing
kdf3 <- subset(kdf, KIR3DL2!=2)
kx3 <- kx[,which(colnames(kx) %in% rownames(kdf3))]
colSums(kx3)
plotMDS(kx3)
...
kx3f <- kx3[which(rowMeans(kx3)>=5),]
dim(kx3f)
dds <- DESeqDataSetFromMatrix(countData = kx3f , colData = kdf3, design = ~ patient_id + KIR3DL2)
res <- DESeq(dds)
z <- results(res)
vsd <- vst(dds, blind=FALSE)
zz <- cbind(as.data.frame(z),assay(vsd),kx3f)
...
kx3de[1:50,1:6] |> kbl(caption="KIR3DL2 associated genes irrespective of HIV status - 2 levels - patient level pairing") |> kable_paper("hover", full_width = F)

write.table(x=kx2de,file="KIR3DL2_de_hivall_2LP.tsv",sep="\t",quote=FALSE)

# 3 levels with patient level pairing
kdf3 <- kdf
kx3 <- kx[,which(colnames(kx) %in% rownames(kdf3))]
colSums(kx3)
plotMDS(kx3)
...
kx3de[1:50,1:6] |> kbl(caption="KIR3DL2 associated genes irrespective of HIV status - 3 levels - patient level pairing") |> kable_paper("hover", full_width = F)

write.table(x=kx2de,file="KIR3DL2_de_hivall_3LP.tsv",sep="\t",quote=FALSE)

```

## Pathway analysiss of gene profiles that correlate with KIR3DL2

This analysis is limited to the 3 level analysis with patient pairing.
...
head(subset(k1m$enrichment_result, p.adjustANOVA < 0.05 & s.dist > 0 ),20) |>
  kbl(caption="top enrichments (positive correl)") |>
  kable_paper("hover", full_width = F)

head(subset(k1m$enrichment_result, p.adjustANOVA < 0.05 & s.dist < 0 ),20)  |>
  kbl(caption="top enrichments (negative correl)") |>
  kable_paper("hover", full_width = F)
...
```{r,kirdl3_pwn_net,fig.width=14,fig.height=9}

networkplot(eres=k1m,FDR=0.05,n_sets=20)
network_genes(eres=k1m,FDR=0.05,n_sets=20)

networkplot(eres=k1m,FDR=0.05,n_sets=30)
network_genes(eres=k1m,FDR=0.05,n_sets=30)
...
head(subset(k2m$enrichment_result, s.dist > 0 & p.adjustANOVA < 0.05 ),20) |>
  kbl(caption="top enrichments (positive correl)")
  |> kable_paper("hover", full_width = F)
...
networkplot(eres=k2m,FDR=0.05,n_sets=20)
network_genes(eres=k2m,FDR=0.05,n_sets=20)

networkplot(eres=k2m,FDR=0.05,n_sets=30)
network_genes(eres=k2m,FDR=0.05,n_sets=30)
...
head(subset(kmm$enrichment_result, p.adjustMANOVA < 0.05),20) |>
  kbl(caption="top multi-enrichment") |>
  kable_paper("hover", full_width = F)
...
head(subset(k3m$enrichment_result, s.dist > 0 & p.adjustANOVA < 0.05),20) |>
  kbl(caption="top enrichments (positive correl)") |>
  kable_paper("hover", full_width = F)
...
```{r,kirdl3_pw2_net,fig.width=13,fig.height=9}

networkplot(eres=k3m,FDR=0.05,n_sets=20)

network_genes(eres=k3m,FDR=0.05,n_sets=20)
